...
## Purpose
Define the guarantees that keep Squid from starting up wedged: the
main-thread discipline every Cocoa call must follow, the `squid doctor`
self-test that proves an install is actually working end to end, and the
pre-commit audit that stops unsafe Cocoa calls from being added.

Synced from the `safe-startup-verification` change on 2026-09-02. That
change remains OPEN: its CI-smoke-test requirement is undecided (blocked
on runner availability) and its launcher-healthcheck requirement was
superseded by the launchd migration -- `squid start` is now a plain
`launchctl bootstrap` and does not run doctor checks. Neither is included
here; both stay in the change until resolved.
...
The `squid` launcher MUST expose a `doctor` subcommand (shelling into
`python -m squid_pet --doctor`) that runs six sequential checks verifying
Squid is functioning end-to-end and reports PASS or FAIL with diagnostics
for each. The checks, in `CHECK_ORDER`, are:

1. process running (from the pid file)
2. `state.json` fresh (the watcher loop is alive)
3. launchd job loaded
4. a visible window for our pid in `CGWindowList`
5. the window is in the expected corner
6. the startup log contains every expected marker

The exit code SHALL be 0 when all pass, otherwise the 1-based index of the
first failing check.

Check 4 SHALL treat a deliberately hidden Squid as healthy rather than
wedged, by consulting the hidden flag before failing on a missing window.
...
- **THEN** the command SHALL print six PASS lines
- **AND** SHALL exit with code 0

#### Scenario: frozen window reports the corner check FAIL
...
#### Scenario: Intentionally hidden Squid is not a failure
- **WHEN** the user has hidden Squid from the menu
- **AND** the user runs `squid doctor`
- **THEN** the window check SHALL NOT report a wedged window

#### Scenario: JSON output for machine consumers
- **WHEN** the user runs `squid doctor --json`
- **THEN** the command SHALL emit one JSON object to stdout with a `checks` array (each entry: `name`, `status`, `diagnostic`)
- **AND** SHALL suppress all human-readable formatting

### Requirement: Pre-commit grep audit blocks unsafe new Cocoa calls

The repository SHALL provide a pre-commit hook (`scripts/check_cocoa_main_thread.py`,
wired as the `cocoa-main-thread-audit` local hook over `src/squid_pet/**.py`)
that fails any commit adding a direct NSWindow/NSApp/NSScreen mutating call
without a `@cocoa_main_thread` decoration on the enclosing function or an
